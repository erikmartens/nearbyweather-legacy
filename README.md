...
Additionally this project is gathering anonymus crash reports and anonymus usage statistics through [Google Firebase](https://firebase.google.com). Google supplies a project configuration file in the form of the `.plist`-file `GoogleService-Info.plist`. This file is not checked into this repository as it contains secrets. In order to run the project remove the file reference from it and uninstall the pod `Firebase/Analytics` by removing the line that starts with `pod 'Firebase/Analytics'` from the `podfile` and executing `pod install`.
